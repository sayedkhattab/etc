                    ...
                    <div class="card-body" style="direction: ltr;">
                        <div class="row">
                            <div class="col-md-9" style="direction: rtl;">
                                <h5 style="color: #597445; text-align: center;">مذكرة الدفاع الأولى</h5>
                                
                                @if(Auth::id() == $case->defendant_id && !$defenseNote)
                                <!-- نموذج لإدخال مذكرة الدفاع -->
                                <form action="{{ route('student.case_details.store_first_defense', $case->id) }}" method="POST" enctype="multipart/form-data">
                                    @csrf
                                    <div class="mb-3">
                                        <label for="content" class="form-label">نص مذكرة الدفاع:</label>
                                        <textarea class="form-control" style="border: solid 1px #dddddd;" id="content" name="content" rows="5" required></textarea>
                                    </div>
                                    <div class="mb-3 text-start">
                                        <label for="attachments" class="form-label">المرفقات:</label>
                                        <input type="file" class="form-control" id="attachments" name="attachments[]" multiple>
                                    </div>
                                    <button type="submit" class="btn btn-success mt-3">حفظ مذكرة الدفاع</button>
                                </form>
                                @endif

                                @if($defenseNote)
                                    <h5 class="mt-5" style="color: #597445;">مذكرة الدفاع:</h5>
                                    <p>{{ $defenseNote->content }}</p>
                                    @if($defenseNote->file_path)
                                        <h5 class="mt-3" style="color: #597445;">المرفقات:</h5>
                                        <ul>
                                            @php
                                                $filename = basename($defenseNote->file_path);
                                            @endphp
                                            <li><a href="{{ route('download.attachment', ['caseId' => $case->id, 'filename' => $filename]) }}" target="_blank">تحميل المذكرة</a></li>
                                        </ul>
                                    @endif
                                @endif
