        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h3 class="card-title mb-0">تعديل دورة: {{ $course->title }}</h3>
                    <a href="{{ route('admin.courses.show', $course) }}" class="btn btn-sm btn-secondary">
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('admin.courses.update', $course) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group mb-3">
                                    ...
                                    <input type="number" step="0.1" name="price" id="price" class="form-control @error('price') is-invalid @enderror" value="{{ old('price', $course->price) }}" required>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group mb-3">
                                    <label for="duration">المدة (بالساعات)</label>
                                    <input type="number" name="duration" id="duration" class="form-control @error('duration') is-invalid @enderror" value="{{ old('duration', $course->duration) }}">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-group mb-3">
                            ...
                            @if($course->thumbnail)
                                <small class="text-muted">الصورة الحالية: {{ $course->thumbnail }}</small>
                            @endif
                        </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group mb-3">
                                    ...
                                    <select name="status" id="status" class="form-select @error('status') is-invalid @enderror" required>
                                        ...
                                        <option value="inactive" {{ old('status', $course->status) == 'inactive' ? 'selected' : '' }}>غير نشطة</option>
                                        <option value="archived" {{ old('status', $course->status) == 'archived' ? 'selected' : '' }}>مؤرشفة</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group mb-3">
                                    ...
                                    <select name="visibility" id="visibility" class="form-select @error('visibility') is-invalid @enderror" required>
                                        <option value="public" {{ old('visibility', $course->visibility) == 'public' ? 'selected' : '' }}>عام</option>
                                        <option value="private" {{ old('visibility', $course->visibility) == 'private' ? 'selected' : '' }}>خاص</option>
                                        <option value="password_protected" {{ old('visibility', $course->visibility) == 'password_protected' ? 'selected' : '' }}>محمي بكلمة مرور</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-group mb-3" id="passwordField" style="display: none;">
                            ...
                            <input type="text" name="access_password" id="access_password" class="form-control @error('access_password') is-invalid @enderror" value="{{ old('access_password', $course->access_password) }}">
                        </div>

                        <div class="form-check mb-4">
                            <input class="form-check-input" type="checkbox" name="featured" id="featured" value="1" {{ old('featured', $course->featured) ? 'checked' : '' }}>
